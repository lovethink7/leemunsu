...
function updateStats() {
  ...
  drawsCount.textContent = String(stats.draws);
...
function playRound() {
  ...
  computerHint.textContent = "컴퓨터가 무작위로 선택했습니다.";
  renderRoundResult(outcome, player, computer, currentBestPrediction.probability);
  updateStats();
}

function refreshCurrentState() {
  roundResult.textContent = "준비 중";
  resultDetail.textContent = "모델이 가장 높은 확률로 인식한 손 모양으로 승패를 계산합니다.";
  computerHand.textContent = "?";
  computerHint.textContent = "랜덤 선택";
}
...
refreshButton.addEventListener("click", refreshCurrentState);
